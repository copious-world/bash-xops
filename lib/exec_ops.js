...
async function expect_ensure_dir(pass,user,addr,remote_dir,remote) {
    let dir_cmd = `mkdir -p ${remote_dir}`
    execFileSync('expect',['./assets/expectpw-ssh-cmd.sh', pass, user, addr, dir_cmd, remote ? '' : 'local' ])
}



async function expect_list_dir(pass,user,addr,remote_dir,remote) {
